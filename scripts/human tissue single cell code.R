# run_all_tissue.R  (English comments)
rm(list = ls())
setwd("/home/dell/Project/General/NEC/single_cell/tissue")   # modify path if needed
raw_data_dir <- "raw"
...
combined$group <- ifelse(combined$orig.ident %in% paste0("NEC-", 1:3), "NEC", "CTRL")

## 7. Rename cell clusters (tissue annotation)  *** exact mapping ***
new.ids <- c(
  "Fibroblasts", "Epithelial_cells", "Endothelial_cells", "T_cells", "Macrophage",
  "B_cell", "Smooth_muscle_cells", "Smooth_muscle_cells", "Epithelial_cells",
  "Fibroblasts", "Neutrophils", "Smooth_muscle_cells", "Neuron", "Epithelial_cells",
  "Epithelial_cells", "Epithelial_cells", "T_cells", "Endothelial_cells",
  "Epithelial_cells", "Smooth_muscle_cells", "Smooth_muscle_cells", "Macrophage",
  "NK_cell", "Smooth_muscle_cells", "Neuron", "Monocyte", "B_cell", "Neuron",
  "Smooth_muscle_cells", "Smooth_muscle_cells", "NK_cell", "B_cell", "Fibroblasts",
  "B_cell", "Neuron", "Neuron", "Neuron"
)
...
ggsave("results/09_vln_kegg_coagPlatelet_tissue.pdf", p.vln, width = 8, height = 5)
...
pheatmap(tissue_T,
         ...
         filename = "results/10_heatmap_geneOrder_tissue.pdf")

## 11. Re-cluster neutrophils
neu <- subset(combined, idents = "Neutrophils")
...
p.cd177 <- FeaturePlot(neu, features = "CD177", split.by = "group",
  ...
  theme_nec + ggtitle("CD177 expression in neutrophils (tissue)")
ggsave("results/11a_feature_cd177_tissue.pdf", p.cd177, width = 6, height = 4)
...
ggsave("results/11b_feature_splitNEC_CTRL_tissue.pdf", p.split, width = 7, height = 4)
...
write_csv(degs %>% rownames_to_column("gene"), "results/12_nec_vs_ctrl_degs_tissue.csv")
...
if (!is.null(kk) && nrow(kk@result) > 0) {
  ...
  p.kegg <- ggplot(kk.df, aes(x = Count, y = reorder(Description, -pvalue),
    ...
    theme_nec
  ggsave("results/13_kegg_nec_vs_ctrl_interferon_tissue.pdf", p.kegg, width = 6, height = 4)
}
...
write_csv(dot.df, "results/14_neu_dot_tissue.csv")
...
ggsave("results/15_neu_dotplot_tissue.pdf", p.dot, width = 7, height = 4)
